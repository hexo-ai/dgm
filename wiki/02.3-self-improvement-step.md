...
```
**Sources:** `[DGM_outer.py:50-150]()`, `[self_improve_step.py:165-300]()`
...
*   **`score_prop`**: Probability based on the `accuracy_score` found in `metadata.json` `[DGM_outer.py:83-90]()`.
*   **`score_child_prop`**: Balances high scores with a penalty for "over-reproduction" (number of existing children) to maintain diversity `[DGM_outer.py:91-100]()`.
*   **`no_darwin`**: A baseline that always selects the most recent commit, simulating a linear improvement path `[DGM_outer.py:79-82]()`.

### Targeted Problem Types
The system identifies specific categories of failure to guide the LLM's diagnosis:
1.  **`solve_empty_patches`**: Targeted when the agent failed to produce any diff for a significant portion of tasks `[DGM_outer.py:125-128]()`.
2.  **`solve_contextlength`**: Triggered if logs indicate "Input is too long" errors (400 Bad Request) `[DGM_outer.py:37-48]()`.
3.  **`solve_stochasticity`**: Aimed at stabilizing flaky performance `[DGM_outer.py:131-134]()`.

**Sources:** `[DGM_outer.py:50-150]()`
...
### 1. `diagnose_problem()`
Used when the system is attempting to fix a failure. It retrieves the evaluation logs for a specific `instance_id` using `find_selfimprove_eval_logs` `[self_improve_step.py:46-55]()`.
...
*   **Input**: Logs and patches from both versions `[self_improve_step.py:100-115]()`.
*   **Process**: It uses the `diagnose_improvement_prompt.py` to identify "regressions" or "successful patterns" that should be merged or refined `[prompts/diagnose_improvement_prompt.py:1-20]()`.

**Sources:** `[self_improve_step.py:41-94]()`, `[self_improve_step.py:97-153]()`
...
1.  **Build**: `build_dgm_container()` creates an image containing the current state of the DGM repository `[utils/docker_utils.py:12-30]()`.
2.  **Run**: The system invokes its own `AgenticSystem.forward()` method, but points it at its own source code `[self_improve_step.py:255-270]()`.
...
2.  The system uses `git_utils.apply_patch()` to create a new commit `[utils/git_utils.py:15-30]()`.
3.  This new commit is added to the `archive` in `dgm_metadata.jsonl`, making it eligible for evaluation and future parent selection `[DGM_outer.py:245-260]()`.

**Sources:** `[self_improve_step.py:165-300]()`, `[utils/docker_utils.py:12-50]()`, `[utils/git_utils.py:1-40]()`
...
**Sources:** `[self_improve_step.py:165]()`, `[self_improve_step.py:41]()`, `[DGM_outer.py:50]()`, `[utils/evo_utils.py:13]()`
